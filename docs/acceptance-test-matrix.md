...
| WL-18 | A task creates representative Comment-eligible Source Files containing functions, methods, language-level procedures, loops, and distinct logical sections. | Add a concise Orientation Comment at each file opening after required preambles and before every created function, method, language-level procedure, loop, and distinct logical section; state purpose or responsibility rather than visible mechanics. | Source inspection maps every applicable boundary to one purpose-bearing comment and confirms required preamble placement. |
| WL-19 | Code created or materially modified within Requested Scope already has an adequate nearby Orientation Comment. | Reuse the existing comment and preserve single-comment coverage. | Changed-source comparison shows adequate coverage and zero duplicate comment introduced for the same role. |
| WL-20 | A required Orientation Comment and a non-required comment merely paraphrase visible syntax, and other comments are duplicate, obsolete, or misleading. | Rewrite the required comment to state purpose or responsibility; remove the non-required mechanical comment and the duplicate, obsolete, or misleading comments while retaining required coverage. | Before-and-after comment inventory, purpose-bearing replacement, removed unnecessary comments, and complete applicable-boundary coverage. |
| WL-21 | Requested work affects a generated, vendored, minified, or comment-incompatible Artifact. | Assign inapplicable comment-insertion status and preserve its source ownership and valid format. | Artifact classification, unchanged comment structure, and syntax or generation Verification. |
| WL-22 | A task materially modifies one source file and one contained function while unrelated functions and code units remain unchanged. | Add or refresh the file-opening and materially modified unit comments while keeping unrelated units outside the Change Surface. | Diff shows required affected-unit coverage and zero comment-only changes to unrelated units. |
| WL-23 | Correct understanding or modification of created or materially modified code depends on non-obvious syntax, format, invariant, Constraint, Side Effect, or rationale information. | Include the material information in the applicable Orientation Comment while keeping the comment concise. | Source inspection identifies the material dependency and its purpose-bearing Orientation Comment. |
| WL-24 | A task needs logic already implemented by a related algorithm, helper, or abstraction. | Inspect the related code before implementation and reuse or extend the existing implementation when it applies to the same algorithm or responsibility. | Changed-source comparison shows one applicable implementation serving the affected current places, with behavior Verification. |
| WL-25 | The same algorithm or responsibility applies to multiple current places affected by a task. | Extract or extend shared code at the narrowest appropriate ownership level while preserving behavior and conventions. | Changed-source comparison shows one shared implementation, updated occurrences, and Verification covering each affected place. |
| WL-26 | A task introduces representative single-use variables, functions, classes, modules, or wrappers that merely rename obvious code or anticipate speculative reuse. | Remove those abstractions during implementation or `Review Code`; retain a single-use abstraction only when it materially improves correctness, clarity, or testability. | Before-and-after source inspection records each abstraction's disposition and confirms preserved behavior. |
| WL-27 | The user asks why an action or disposition occurred. | Apply `Reason From Evidence` and explain it in plain language from the available applicable instructions, task decisions, Evidence Items, and Procedure Execution Records; identify historical information as historical and report any unavailable basis. | Explanation maps the action or disposition to its available recorded basis, labels historical information, and identifies missing support. |
